      ...
      <div class="window-controls">
        <button class="control-btn theme-toggle" @click="toggleTheme" 
          ...
          <svg v-if="theme === 'dark'" viewBox="0 0 24 24" fill="none">
            ...
            <path d="M12 2v2M12 20v2M4 12H2M22 12h-2M6.34 6.34L4.93 4.93M19.07 4.93l-1.41 1.41M6.34 17.66l-1.41 1.41M19.07 19.07l-1.41-1.41" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
          </svg>
      ...
      <div class="panel-section">
          ...
          <div class="source-selector" v-if="sources.length > 0">
            ...
            >
              <div class="source-thumb">
                <VideoPreview :source-id="source.id" :active="config.sourceId === source.id" />
              </div>
        ...
        <div class="setting-item">
          <label class="setting-label">
            ...
            录制码率
            <span class="badge primary">{{ formatBitrate(config.bitRate) }}</span>
          ...
          <el-slider
            ...
            :step="5000"
            :disabled="isRecording"
            :marks="{ 5000: '5M', 50000: '50M', 100000: '100M', 120000: '120M' }"
          />
        </div>
        ...
        <div class="sub-section">
          ...
          <div class="setting-item">
            <label class="setting-label">
              ...
              回溯码率
              <span class="badge primary">{{ formatBitrate(config.replayBitRate) }}</span>
              <span class="size-estimate">≈ {{ estimatedReplaySizePerMin }}/min</span>
            </label>
            <el-slider
        ...
        <div class="setting-item">
          ...
          <div class="path-input-group">
            <input type="text" v-model="config.savePath" :disabled="isRecording" readonly />
      ...
      <div class="panel-section monitor-section">
        ...
        <div class="monitor-item">
          <div class="monitor-label">
            <span>CPU 使用率</span>
            <span class="monitor-value">{{ (sysUsage.cpuUsage * 100).toFixed(1) }}%</span>
          </div>
          <el-progress
            :percentage="isNaN(sysUsage.cpuUsage) ? 0 : Math.min(100, Math.round(sysUsage.cpuUsage * 100))"
            :show-text="false"
            :stroke-width="4"
            stroke-linecap="round"
          />
        ...
        <div class="monitor-item">
          ...
          <el-progress
            :percentage="(!totalMemGB || isNaN(usedMemGB / totalMemGB)) ? 0 : Math.round((usedMemGB / totalMemGB) * 100)"
            :show-text="false"
            :stroke-width="4"
            stroke-linecap="round"
            status="warning"
        ...
        <div class="monitor-item" v-if="diskInfo">
          ...
          <el-progress
            :percentage="(!diskInfo || !diskInfo.size || isNaN((diskInfo.size - diskInfo.free) / diskInfo.size)) ? 0 : Math.round(((diskInfo.size - diskInfo.free) / diskInfo.size) * 100)"
            :show-text="false"
            :stroke-width="4"
            stroke-linecap="round"
            status="success"
...
// 回溯录制缓冲区
const replayBuffer = ref<{ blob: Blob; timestamp: number }[]>([])
const replayRecorder = shallowRef<MediaRecorder | null>(null)
const replayStream = shallowRef<MediaStream | null>(null)
const REPLAY_DURATION = 120 * 1000 // 120秒
...
// ==================== 计算属性 ====================
const usedMemGB = computed(() => (sysUsage.value.totalMem - sysUsage.value.freeMem) / (1024 * 1024 * 1024))
const totalMemGB = computed(() => sysUsage.value.totalMem / (1024 * 1024 * 1024))
...
function getMimeType(format: string): string {
  ...
  if (format === 'mkv') {
       if (MediaRecorder.isTypeSupported('video/x-matroska;codecs=avc1')) return 'video/x-matroska;codecs=avc1'
...
async function loadConfig() {
  try {
    ...
    if (savedConfig.replayBitRate) config.replayBitRate = savedConfig.replayBitRate
...
async function refreshSources() {
  try {
     ...
     const screenSources = _sources.filter(s => s.name.toLowerCase().includes('screen') || s.name.includes('屏幕') || s.id.startsWith('screen:'))
     sources.value = screenSources.length > 0 ? screenSources : _sources
     
     if (!sources.value.find(s => s.id === config.sourceId) && sources.value.length > 0) {
...
async function getDisplayMediaStream(): Promise<MediaStream> {
  ...
  try {
      ...
      console.warn('Failed with primary constraints, retrying with minimal...', err)
      const fallbackConstraints = {
          audio: false,
          video: { mandatory: { chromeMediaSource: 'desktop', chromeMediaSourceId: config.sourceId, minFrameRate: config.frameRate, maxFrameRate: 60 } }
      } as any
...
async function startRecording() {
  ...
  try {
    ...
    window.electronAPI.updateRecordingStatus({ isRecording: true, isReplay: false })
...
async function stopRecording() {
  ...
  window.electronAPI.updateRecordingStatus({ isRecording: false, isReplay: false })
...
async function saveRecording() {
  ...
  const blob = new Blob(recordedChunks.value, { type: getMimeType(config.format) })
  let finalBlob = blob
  ...
  if (config.format === 'webm' || config.format === 'mkv') {
      ...
      try {
        ...
        console.warn(`Failed to fix ${config.format} duration, saving original:`, e)
      }
  }
...
async function startReplayBuffer() {
  ...
  try {
    ...
    replayCleanupTimer = setInterval(() => {
      ...
      replayBuffer.value = replayBuffer.value.filter(c => c.timestamp > cutoff)
    }, 10000)
...
async function saveReplayRecording() {
  ...
  window.electronAPI.updateRecordingStatus({ isRecording: false, isReplay: true })
...
.status-dot.idle { background-color: var(--success); }
.status-dot.recording { background-color: var(--danger); }
.status-dot.replay-recording { background-color: var(--warning); }
